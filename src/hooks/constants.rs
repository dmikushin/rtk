...
/// Where the Claude Code settings file holds the PostToolUse hook list.
/// The old layer hooked PreToolUse and rewrote the command before the shell
/// ran it, which put rtk between the program and the file the command wrote.
/// The PostToolUse hook condenses only the captured response, after the
/// command ran untouched.
pub const POST_TOOL_USE_KEY: &str = "PostToolUse";
pub const BEFORE_TOOL_KEY: &str = "BeforeTool";

/// Native Rust hook command for Claude Code (replaces the old PreToolUse
/// `rtk hook claude` rewriter).
pub const CLAUDE_HOOK_COMMAND: &str = "rtk hook post-tool-use";
